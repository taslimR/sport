		...
		function render_html( $username, $numbers, $cache_time = 1200, $follow = true ) {
			...
			if ( is_null( $pins ) ) {
				...
				foreach ( $pins as $pin ) {
					...
					echo '<a href="' . esc_url( $url ) . '" title="' . esc_attr( $title ) . '" target="_blank" alt="' . esc_attr( $title ) . '"><span class="penci-image-holder rectangle-fix-size" style="background-image: url(' . esc_url( $image ) . ');"></span></a>';
				}
			}
			?>
			</div>
