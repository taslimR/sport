...
<div class="featured-area magazine-slider">
	<div class="penci-magazine-slider penci-mag1-slider penci-loading" data-direction="horizontal" data-auto="<?php echo esc_attr( $data_auto ); ?>" data-autotime="<?php echo esc_attr( $auto_time ); ?>" data-speed="<?php echo esc_attr( $auto_speed ); ?>">
		<?php
		...
		<ul class="slides">
			<?php $i = 1; if ( $feat_query->have_posts() ) : while ( $feat_query->have_posts() ) : $feat_query->the_post(); ?>
				...
				<li class="mag-item-<?php echo esc_attr( $count ); ?>">
					...
					<div class="mag-img" style="background-image: url('<?php echo penci_get_featured_image_size( get_the_ID(), $thumb ); ?>');" title="<?php echo wp_strip_all_tags( get_the_title() ); ?>">
					</div>
