...
/* Add google fonts to head tag */
function pencidesign_enqueue_google_fonts(){
	/* Enqueue font for title */
	if( get_theme_mod( 'penci_font_for_title' ) && '"Raleway", "100:200:300:regular:500:600:700:800:900", sans-serif' != get_theme_mod( 'penci_font_for_title' ) ) {
		...
		if ( ! in_array( $font_family_title, penci_font_browser() ) ) {
			...
			wp_enqueue_style( 'penci_font_title', (is_ssl() ? 'https://' : 'http://') . 'fonts.googleapis.com/css?family=' . $title_font_replace . ':' . $font_title_type,'', '1.0' );
		}
	}

	/* Enqueue font for body */
	if( get_theme_mod( 'penci_font_for_body' ) && '"PT Serif", "regular:italic:700:700italic", serif' != get_theme_mod( 'penci_font_for_body' ) ) {
		...
		if( !in_array( $font_family_body, penci_font_browser() ) ) {
			...
			wp_enqueue_style( 'penci_font_body', (is_ssl() ? 'https://' : 'http://') . 'fonts.googleapis.com/css?family=' . $body_font_replace . ':' . $font_body_type,'', '1.0' );
		}
	}

	/* Enqueue font for slogan */
	if( get_theme_mod( 'penci_font_for_slogan' ) && '"PT Serif", "regular:italic:700:700italic", serif' != get_theme_mod( 'penci_font_for_slogan' ) ) {
		...
		if( !in_array( $font_family_slogan, penci_font_browser() ) ) {
			...
			$font_slogan_type = $font_slogan_explo[1];
			$font_slogan_type = str_replace(':',',',$font_slogan_type);
			$slogan_font_replace = str_replace( ' ', '+', $font_slogan );

			wp_enqueue_style( 'penci_font_slogan', (is_ssl() ? 'https://' : 'http://') . 'fonts.googleapis.com/css?family=' . $slogan_font_replace . ':' . $font_slogan_type,'', '1.0' );
		}
	}
}
add_action( 'wp_enqueue_scripts', 'pencidesign_enqueue_google_fonts' );
...
function pencidesign_customizer_css() {
    ...
    <style type="text/css">
		...
		?>
		body, textarea, #respond textarea, .widget input[type="text"], .widget input[type="email"], .widget input[type="date"], .widget input[type="number"], .wpcf7 textarea, .mc4wp-form input, #respond input, .wpcf7 input, #searchform input.search-input, ul.homepage-featured-boxes .penci-fea-in h4, .widget.widget_categories ul li span.category-item-count, .about-widget .about-me-heading, .widget ul.side-newsfeed li .side-item .side-item-text .side-item-meta { font-family: <?php echo sanitize_text_field( $font_family_body_end ); ?>;  }
		<?php
		}
		?>
		<?php
		...
		.footer-instagram h4.footer-instagram-title > span:before, .woocommerce nav.woocommerce-pagination ul li span.current, .penci-pagination.penci-ajax-more a.penci-ajax-more-button:hover, .penci-recipe-heading a.penci-recipe-print:hover { border-color: <?php echo get_theme_mod( 'penci_color_accent' ); ?>; }
		...
		.header-social a i { font-size: <?php echo get_theme_mod( 'penci_size_header_social_check' ); ?>px; }
		<?php endif; ?>
		...
		.penci-sidebar-content .penci-border-arrow .inner-arrow { border-color: <?php echo get_theme_mod( 'penci_sidebar_heading_border_color' ); ?>; }
		...
		.widget ul.side-newsfeed li .side-item .side-item-text h4 a:hover, .widget a:hover, .penci-sidebar-content .widget-social a:hover span, .widget-social a:hover span, .penci-tweets-widget-content .icon-tweets, .penci-tweets-widget-content .tweet-intents a, .penci-tweets-widget-content .tweet-intents span:after { color: <?php echo get_theme_mod( 'penci_sidebar_accent_hover_color' ); ?>; }
